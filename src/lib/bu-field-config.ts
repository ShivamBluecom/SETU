...
export const BU_FIELD_CONFIG: Record<string, BUConfig> = {
  ISG: {
    ...
    fields: [
      { key: 'subcategory', label: 'Subcategory', type: 'select', options: ISG_SUBCATEGORIES, required: false },
      { key: 'oem', label: 'OEM', type: 'select', oemManaged: true, required: false },
      { key: 'mtmPartNumber', label: 'MTM / Part Number', type: 'text', required: false, placeholder: 'e.g. 20XW0044IN' },
      { key: 'productDescription', label: 'Product Description', type: 'text', required: false },
  ...
  NETWORKING_AV: {
    ...
    totalValueFormula: 'QTY_X_UNIT',
    fields: [
      { key: 'subcategory', label: 'Subcategory', type: 'select', options: NETWORKING_SUBCATEGORIES, required: false },
      { key: 'oem', label: 'OEM', type: 'select', oemManaged: true, required: false },
      { key: 'mtmPartNumber', label: 'MTM / Part Number', type: 'text', required: false, placeholder: 'e.g. C9200L-24T-4G-E' },
      { key: 'productDescription', label: 'Product Description', type: 'text', required: false },
      { key: 'warrantySupportTier', label: 'Warranty / Support Tier', type: 'text', required: false, placeholder: 'e.g. SmartNet 8×5×NBD' },
      { key: 'warrantyPeriod', label: 'Warranty Period', type: 'select', options: WARRANTY_PERIODS, required: false },
  ...
  ISS: {
    ...
    totalValueFormula: 'QTY_X_UNIT',
    fields: [
      { key: 'subcategory', label: 'Subcategory', type: 'select', options: ISS_SUBCATEGORIES, required: false },
      { key: 'oem', label: 'OEM', type: 'select', oemManaged: true, required: false },
      { key: 'mtmPartNumber', label: 'MTM / Part Number', type: 'text', required: false },
      { key: 'productDescription', label: 'Product Description', type: 'text', required: false },
      { key: 'licenceCount', label: 'Licence Count', type: 'number', required: false, placeholder: 'e.g. 100' },
      { key: 'subscriptionTerm', label: 'Subscription Term', type: 'select', options: SUBSCRIPTION_TERMS, required: false },
      { key: 'renewalDate', label: 'Renewal Date', type: 'date', required: false, hint: 'Date format: DD/MM/YYYY' },
      { key: 'deploymentType', label: 'Deployment Type', type: 'select', options: DEPLOYMENT_TYPES, required: false },
      { key: 'opportunityType', label: 'Opportunity Type', type: 'select', options: OPPORTUNITY_TYPES, required: false },
  ...
  SSG: {
    ...
    fields: [
      { key: 'subcategory', label: 'Subcategory', type: 'select', options: SSG_SUBCATEGORIES, required: false },
      { key: 'oem', label: 'OEM', type: 'select', oemManaged: true, required: false },
      { key: 'skuId', label: 'SKU ID', type: 'text', required: false, placeholder: 'e.g. CFQ7TTC0LH18:0001', hint: 'Microsoft SKU / part number from price list' },
      { key: 'productDescription', label: 'Product Description', type: 'text', required: false },
      { key: 'licenceType', label: 'Licence Type', type: 'select', options: LICENCE_TYPES, required: false },
      { key: 'renewalDate', label: 'Renewal Date', type: 'date', required: false, hint: 'Date format: DD/MM/YYYY' },
      { key: 'opportunityType', label: 'Opportunity Type', type: 'select', options: OPPORTUNITY_TYPES, required: false },
      { key: 'oemDrNumber', label: 'OEM DR Number', type: 'text', required: false },
    ],
  ...
  CLOUD: {
    ...
    fields: [
      { key: 'opportunityType', label: 'Opportunity Type', type: 'select', options: CLOUD_OPPORTUNITY_TYPES, required: false },
      { key: 'cloudPlatform', label: 'Cloud Platform', type: 'select', options: CLOUD_PLATFORMS, required: false },
      { key: 'projectOverview', label: 'Project Overview', type: 'textarea', required: false },
      { key: 'customerProblem', label: 'Customer Problem', type: 'textarea', required: false },
      { key: 'commitmentTerm', label: 'Commitment Term', type: 'select', options: COMMITMENT_TERMS, required: false },
      { key: 'billingModel', label: 'Billing Model', type: 'select', options: BILLING_MODELS, required: false },
      { key: 'monthlyCommitValue', label: 'Monthly Commit Value ₹', type: 'number', required: false, placeholder: '0' },
