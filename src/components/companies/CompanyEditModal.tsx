...
export function CompanyEditModal({ open, onOpenChange, company, onSaved }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    userCount: company.userCount != null ? String(company.userCount) : '',
  ...
  // Re-sync form when company prop changes (after a save + router.refresh)
  useEffect(() => {
    setForm({
      name: company.name,
      industry: company.industry ?? '',
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (err.field === 'name') {
          ...
          showToast(err.error ?? 'Duplicate company', 'error')
        }
  ...
  const borderColor = (field: string): string | undefined =>
    errors[field] ? 'var(--color-danger)' : undefined

  return (
    <Modal open={open} onOpenChange={(v) => { onOpenChange(v); if (!v) setErrors({}) }} title="Edit Company" maxWidth="580px">
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
          ...
          <div>
            ...
            <select value={form.territoryId} onChange={set('territoryId')} required style={{ borderColor: borderColor('territoryId') }}>
              <option value="">Select…</option>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', paddingTop: '8px' }}>
          <button type="button" className="btn-secondary" onClick={() => { onOpenChange(false); setErrors({}) }}>Cancel</button>
